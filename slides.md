...
We choose Rust to Lean via Aeneas because it is the most relevant combination today

---

## A Rust crate of cyptographic primatives: curve25519-dalek

- High performance, open source cryptographic primitives (elliptic curve cryptography) code 
...
- Array of 5 64-bit limbs representing numbers in radix 2^51
    ...
    ```
- Allows "space" to do calculations in place, growing up to `2^54` between reductions.
...
fn reduce(mut limbs: [u64; 5]) -> FieldElement51 {
    ...
    // ... mask and add carries ...
    limbs[0] += c4 * 19;  // key: wraps around mod p
...
- Field element: 5 limbs, `∑ i ∈ range 5, 2^(51*i) * limbs[i]`
...
## Example function: `reduce` - Lean spec

```lean
/-- Curve25519 is the elliptic curve over the prime field with order p -/
def p : Nat := 2^255 - 19

/-- Interpret a Field51 as a natural number -/
...
- All limbs of result are small, ≤ 2^(51 + ε)
- Requires input limbs bounded to avoid underflow -/
@[progress]
theorem negate_spec (r : FieldElement51) (h : ∀ i < 5, r[i]!.val < 2 ^ 54) :
  ...
  · have : 16 * p =
    ...
    simp_all [Nat.ModEq, Field51_as_Nat, ...]
    grind
  · assumption
```
...
Plus conversion functions between them
...
1. **The Lean Proof Checker**
   - Lean's minimal trusted kernel guarantees absolute correctness

2. **The Aeneas Translation**
   - Lean code extracted by Aeneas faithfully represents the Rust code
   - Limited set of translations, subject to ongoing soundness proofs
...
4. **External specifications**
   ...
   - These files need inspection to ensure correctness
...
- E.g., `a = limbs.set i i2` - awkward naming
- E.g., `>>>` with `grind` - tactic limitations
- E.g., proofs depend on variable names introduced by tactic - makes proof fragile
- E.g., uses `Aeneas.Std.Array` which isn't standard - don't benefit from established API
...
- Can start to do the proof manually, explore the proof, use simp, etc.
